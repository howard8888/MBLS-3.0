...
#--------------------------------------
#-New Python programmer? No need to install anything -- Python will automatically import.
...
#----------------------------------
#-New Programmer note: Many of these such as, eg, numpy are widely used and are easy to find,
#often included with many Python distributions, eg, Anaconda; all others are easily installed
#via PyPi.org Python Package Index -- simply copy and run at the command line the 'pip' info
#from PyPi and the module will be installed on your computer.
#-Any module not in PyPi will be available on or via the MBLS GitHub page.
#-Style note: Import well known packages such as numpy with abbreviation, since to brain it
#is not really an abbreviation.
...
#-New Python programmer? Make sure these files are in the same directory where your Python
#program is running or is in a path which Python can follow to find these files.
...
#Ebbinghaus number of the human mind, then we will replace the abbreviations with the full
#module name.
...
def sleep_selection(sleep_phase: int) -> int:
    ...
    if sleep_phase not in (1, 2, 3, 5):
        print('Possible coding error: sleep phase parameter {} entered is not a sleep inducing '
